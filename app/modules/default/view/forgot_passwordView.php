        ...
        <div class="card-body login-body-custom">
            ...
            <?php if(isset($data['result'])): ?>
                <?php if($data['result']['success']): ?>
                    <div class="alert alert-success p-3 mb-4 shadow-sm" style="border-radius: 8px;">
                        <h6 class="font-weight-bold mb-1"><i class="fas fa-check-circle mr-1"></i> <?= Lang::isTr() ? 'Talep Başarılı!' : 'Request Successful!' ?></h6>
                        <p class="small mb-0"><?= htmlspecialchars($data['result']['message']) ?></p>
                    </div>
                <?php else: ?>
                    <div class="alert alert-danger p-2 mb-4 text-center small" style="border-radius: 8px;">
                        <i class="fas fa-exclamation-circle mr-1"></i> <?= htmlspecialchars($data['result']['message']) ?>
                    </div>
                <?php endif; ?>
            <?php else: ?>
                <p class="text-center text-muted mb-4 small"><?= __('forgot_password_desc') ?></p>
            <?php endif; ?>

            <form action="/default/forgot_password" method="post">
                ...
                <button type="submit" class="btn btn-primary btn-block btn-action-custom text-white w-100 fs-5 mb-3 shadow">
                    <?= __('send_reset_link') ?> <i class="fas fa-paper-plane ms-2"></i>
                </button>
