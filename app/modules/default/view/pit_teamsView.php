            ...
            <div class="deep-dive-header mb-4 d-flex justify-content-between align-items-center">
                <div class="d-flex flex-column pe-3">
                    <h2 class="m-0 fw-bold fs-4 fs-md-3 text-truncate">
                        <i class="fas fa-tools text-warning me-2"></i> <?= __('pit_teams_title') ?>
                    </h2>
                    <div class="mt-2">
                ...
                if (!empty($data['takimlar'])):
                                    ...
                                    <div class="mt-auto">
                                        <?php if ($isScouted): ?>
                                            <button class="btn btn-sm btn-outline-success w-100 fw-bold rounded-pill" disabled style="opacity: 0.85;">
                                                <i class="fas fa-check-double"></i> <?= __('pit_done') ?>
                                            </button>
                                        <?php else: ?>
                                            <a href="/default/pit_scout/<?= $team['key'] ?>/<?= $data['secilen_turnuva'] ?>"
                                               class="btn btn-sm btn-warning w-100 fw-bold rounded-pill shadow-sm" style="color: #212529;">
                                                <?= __('open_pit_form') ?> <i class="fas fa-wrench ms-1" style="font-size: 0.75rem;"></i>
                                            </a>
                                        <?php endif; ?>
                                    </div>
                    ...
                    <div class="col-12">
                        <div class="alert alert-warning shadow-sm rounded-3 p-4 text-center">
                            ...
                            <h5 class="fw-bold text-dark"><?= Lang::isTr() ? 'Takım Listesi Bulunamadı' : 'No Teams Found' ?></h5>
                            <p class="text-muted mb-0"><?= Lang::isTr() ? 'Bu turnuvaya ait takım listesi TBA üzerinde henüz yayınlanmamış.' : 'No teams were found for this event or TBA roster data is currently unavailable.' ?></p>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
